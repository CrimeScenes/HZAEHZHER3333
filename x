...
if getgenv().Forbidden.Sentinel.CleanConsole.Enabled and getgenv().Forbidden.Sentinel.CleanConsole.Mode == "auto" then
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if getgenv().Forbidden.Sentinel.CleanConsole.Mode == "keybind" and input.KeyCode == Enum.KeyCode[string.upper(getgenv().Forbidden.Sentinel.KeyTrigger.ClearConsole)] then
        shouldClear = true
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode[string.upper(getgenv().Forbidden.Sentinel.KeyTrigger.ClearConsole)] then
...
if getgenv().Forbidden.Sentinel.Memory.IsActive then
    local Memory
    ...
    task.spawn(function()
        while task.wait(1) do
            local minMemory = getgenv().Forbidden.Sentinel.Memory.Params.MinValue
            local maxMemory = getgenv().Forbidden.Sentinel.Memory.Params.MaxValue
...
local function isWithinBoxFOV(position)
    ...
    local fovHeight = getgenv().Forbidden.Sentinel.BoxFov.Height * 100
    local fovWidth = getgenv().Forbidden.Sentinel.BoxFov.Width * 100
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode[string.upper(getgenv().Forbidden.Sentinel.KeyTrigger.TriggerBot)] then
        isToggled = true
    end
end)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode[string.upper(getgenv().Forbidden.Sentinel.KeyTrigger.Visuals)] then
...
local function updateTracer()
    if TargetPlayer and TargetPlayer.Character then
        ...
        if getgenv().Forbidden.Sentinel.Visuals.TargetClosestBodyPart then
        ...
        if targetPart then
            if not TracerLine then
                ...
                TracerLine.Thickness = getgenv().Forbidden.Sentinel.Visuals.TracerThickness
                TracerLine.Transparency = getgenv().Forbidden.Sentinel.Visuals.TracerTransparency
            end
...
local function handleESP()
    if visualsActive and getgenv().Forbidden.Sentinel.Visuals.Enabled then
...
local function getDynamicFov(targetPosition)
    ...
    if distance <= getgenv().Forbidden.Sentinel.Ranges.CloseDistance then
        return getgenv().Forbidden.Sentinel.Silent.BoudingSizes.CloseFovRadius
    elseif distance <= getgenv().Forbidden.Sentinel.Ranges.MidDistance then
        return getgenv().Forbidden.Sentinel.Silent.BoudingSizes.MidFovRadius
    elseif distance <= getgenv().Forbidden.Sentinel.Ranges.FarDistance then
        return getgenv().Forbidden.Sentinel.Silent.BoudingSizes.FarFovRadius
    else
        return getgenv().Forbidden.Sentinel.Silent.BoudingSizes.FarFovRadius  
    end
end
...
local function updateFov(TargetPlayer)
    local settings = getgenv().Forbidden.Sentinel.Silent.BoudingSizes
    clearFovParts()
...
local function isPartInFovAndVisible(part)
    if not getgenv().Forbidden.Sentinel.CamLock.Normal.Enabled or not IsTargeting or not TargetPlayer then
        return false
    end
...
local function isPartVisible(part)
    if not getgenv().Forbidden.Sentinel.TargetControl.Safety then 
        return true  
    end
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        if velocity.Y < -30 and getgenv().Forbidden.Sentinel.TargetControl.Aligner then
            getgenv().Forbidden.Sentinel.Silent.Prediction = 0
            return velocity
        elseif velocity.Magnitude > 50 and getgenv().Forbidden.Sentinel.TargetControl.Aligner then
            return player.Character:FindFirstChild("Humanoid").MoveDirection * 16
        else
            getgenv().Forbidden.Sentinel.Silent.Prediction = getgenv().Forbidden.Sentinel.Silent.Prediction -- Preserve old prediction value
            return velocity
        end
    end
...
UserInputService.InputEnded:Connect(function(input, isProcessed)
    if input.KeyCode == Enum.KeyCode[getgenv().Forbidden.Sentinel.Target.Keybind:upper()] and CamLock.Normal.mode == "hold" then
        holdingMouseButton = false
    end

    if input.KeyCode == Enum.KeyCode[getgenv().Forbidden.Sentinel.Target.UntargetKeybind:upper()] then
        ResetTargeting()
    end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    if getgenv().Forbidden.Sentinel.Silent.Enabled and IsTargeting then  
        UpdateFOV()  

        if getgenv().Forbidden.Sentinel.Silent.mode == "target" and TargetPlayer then
...
task.spawn(function()
    while task.wait(0.1) do
        if getgenv().Forbidden.Sentinel.Silent.Enabled then
            Fov.Visible = IsTargeting and getgenv().Forbidden.Sentinel.Silent.AimSettings.FovSettings.FovVisible  
        end
    end
end)
...
if getgenv().Forbidden.Sentinel.Adjustment.Checks.NoGroundShots == true then
    local function CheckNoGroundShots(Plr)
        if getgenv().Forbidden.Sentinel.Adjustment.Checks.NoGroundShots and Plr.Character:FindFirstChild("Humanoid") and Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            pcall(function()
                local TargetVelv5 = Plr.Character:FindFirstChild(getgenv().Forbidden.Sentinel.Silent and getgenv().Forbidden.Sentinel.Silent)
                if TargetVelv5 then
